...
## Editorial
The direct approach would inspect every denominator $d$, generate the numerators in the interval $(d/3, d/2)$, and then filter by $\gcd(n,d)=1$. The Mobius identity lets us perform that filtering in bulk instead of checking gcds one fraction at a time.

After expanding the coprimality condition with $\mu$, the problem reorganizes into divisor classes indexed by $k$. For a fixed $k$, we only need to count how many raw numerators fall into the interval for denominators up to $\lfloor N/k \rfloor$; the Mobius value $\mu(k)$ then decides whether that whole class contributes positively, negatively, or not at all. So the candidates are generated by the interval bounds for each denominator, while the Mobius weights perform the filtering that enforces reduced fractions.
