...
\begin{itemize}
    \item \textbf{Enumeration approach:} $O(N^2 \log N)$ time to enumerate all lines in an $N \times N$ grid.
    \item \textbf{Counting:} $O(L)$ where $L$ is the number of lines, each requiring a binomial coefficient computation.
\end{itemize}
