...
1. **Decomposition:** Break the problem into sub-problems using the DP over digit positions structure.
...
We reformulate the counting/optimization problem in terms of DP over digit positions. This transformation preserves the answer while exposing the algebraic structure.

**Step 2: Efficient Evaluation.**
Using Miller-Rabin primality, we evaluate the reformulated expression. The key observation is that the naive $O(N^2)$ approach can be improved to $O(N * M)$ by exploiting:
...
2. **Main computation:** Apply the Miller-Rabin primality to evaluate the target quantity.
