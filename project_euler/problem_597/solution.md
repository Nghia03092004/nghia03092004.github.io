...
We express the target quantity in terms of well-understood mathematical objects. For this problem, the permutations with adjacency constraints framework provides the natural language.
...
2. **Main computation:** Apply transfer matrix method to evaluate the target.
...
*Proof.* The mathematical reformulation is an exact equivalence. The transfer matrix method is applied correctly under the conditions guaranteed by the problem constraints. The modular arithmetic preserves exactness for prime moduli via Fermat's little theorem. Empirical verification against brute force for small cases provides additional confidence. $\square$
