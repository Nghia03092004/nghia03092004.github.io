...
### Core Framework: Inclusion-Exclusion On Collision Events

The solution hinges on **inclusion-exclusion on collision events**. We develop the mathematical framework step by step.

### Key Identity / Formula

The central tool is the **generating functions for occupancy**. This technique allows us to:
...
We express the target quantity in terms of well-understood mathematical objects. For this problem, the inclusion-exclusion on collision events framework provides the natural language.
...
- The generating functions for occupancy applies because the underlying objects satisfy a decomposition property.
- Sub-problems of size $n/2$ (or $\sqrt{n}$) can be combined in $O(1)$ or $O(\log n)$ time.

**Step 3 (Efficient Evaluation).**
Using generating functions for occupancy:
...
*Proof.* The mathematical reformulation is an exact equivalence. The generating functions for occupancy is applied correctly under the conditions guaranteed by the problem constraints. The modular arithmetic preserves exactness for prime moduli via Fermat's little theorem. Empirical verification against brute force for small cases provides additional confidence. $\square$
