...
## Editorial
The expensive part is not the final selection but computing all radicals efficiently. A sieve handles that cleanly: each time a prime $p$ is discovered, multiply $p$ into the radical of every multiple of $p$. Because each prime contributes only once to each number, the resulting array is exactly $\operatorname{rad}(n)$ for every $1 \le n \le 100000$.

After that, the rest is a pure ordering problem. Form the pairs $(\operatorname{rad}(n), n)$, sort them lexicographically, and read off the entry in position 10000. The secondary key $n$ automatically resolves ties between equal radicals.

## Pseudocode

```text
Initialize every radical value to 1.

Sweep through the integers from 2 up to 100000.
Whenever the current integer is still marked with radical 1, treat it as prime and multiply it into the radical of each of its multiples.

Build the list of pairs (rad(n), n) for all n in the range.
Sort that list in lexicographic order and return the n-value in position 10000.
```
