...
```
function ComputeG(K, m):
    // K = 10^18, m = 20092010
    // Compute x^K mod p(x) in Z_m[x], where p(x) = x^2000 - x - 1

    function PolyMultMod(A, B, m):
        // Multiply polynomials A, B (each degree < 2000)
        // Result C has degree < 4000
        C = ConvolveCoefficients(A, B)  // O(n^2) where n = 2000
        // Reduce C modulo p(x): for d = deg(C) down to 2000:
        //   C[d-1999] += C[d]
        //   C[d-2000] += C[d]
        //   C[d] = 0
        //   All operations mod m
        for d = len(C)-1 downto 2000:
            C[d-1999] = (C[d-1999] + C[d]) mod m
            C[d-2000] = (C[d-2000] + C[d]) mod m
            C[d] = 0
        return C[0..1999]

    // Binary exponentiation
    result = [1, 0, 0, ..., 0]   // polynomial "1"
    base = [0, 1, 0, ..., 0]     // polynomial "x"

    while K > 0:
        if K is odd:
            result = PolyMultMod(result, base, m)
        base = PolyMultMod(base, base, m)
        K = K >> 1

    // Extract answer: sum of coefficients
    answer = sum(result[0..1999]) mod m
    return answer
```
