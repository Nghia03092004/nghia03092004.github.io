...
\title{Problem 426: Box-ball System}
...
\begin{itemize}
    \item \textbf{Soliton decomposition:} $O(n)$ where $n$ is the configuration length.
    \item \textbf{Time evolution:} $O(k)$ per soliton for large $t$, with $O(k^2)$ for phase shift computation.
\end{itemize}
