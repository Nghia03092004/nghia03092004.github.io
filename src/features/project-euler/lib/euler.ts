...
import { getEulerMetadata } from './euler-metadata';
...
async function loadEulerRecords() {
	...
	const metadata = await getEulerMetadata();
	...
	const baseRecords = await Promise.all(
		entries.map(async (entry) => {
			...
			const latexPath = path.join(directory, SOURCE_FILE_NAMES.tex);
			const stats = await fs.stat(solutionPath);
			const hasLatexSource = await fs
				.access(latexPath)
				.then(() => true)
				.catch(() => false);
			...
			return {
				...
				tags: metadata.get(problem)?.tags ?? [],
				languages: ['C++', 'Python'],
				entry,
				hasLatexSource,
				sourcePaths: {
					...
					tex: path.relative(process.cwd(), latexPath),
				},
