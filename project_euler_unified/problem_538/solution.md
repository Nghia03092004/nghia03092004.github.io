...
We reformulate the counting/optimization problem in terms of convex hull + rotating calipers. This transformation preserves the answer while exposing the algebraic structure.

**Step 2: Efficient Evaluation.**
Using shoelace formula, we evaluate the reformulated expression. The key observation is that the naive $O(N^2)$ approach can be improved to $O(n log n + h^2)$ by exploiting:
...
2. **Main computation:** Apply the shoelace formula to evaluate the target quantity.
