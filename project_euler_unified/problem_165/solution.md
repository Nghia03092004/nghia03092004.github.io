...
```
function COUNT_INTERIOR_INTERSECTIONS():
    ...
    for i = 1 to 5000:
        ...
        for j = 1 to 4:
            s = (s * s) mod 50515093
            coords.append(s mod 500)
        segments.append(Segment(coords[0], coords[1], coords[2], coords[3]))

    # Find all interior intersection points
    points = Set()   # stores (p, q, r) triples in lowest terms
    ...
    for i = 0 to 4998:
        for j = i+1 to 4999:
            ...
            if D > 0:
                if not (0 < t_num < D): continue
                if not (0 < u_num < D): continue  # note: sign of u formula
            else:
                if not (D < t_num < 0): continue
                if not (D < u_num < 0): continue
            ...
            g = gcd(gcd(abs(px), abs(py)), abs(D))
            points.add((px/g, py/g, D/g))
